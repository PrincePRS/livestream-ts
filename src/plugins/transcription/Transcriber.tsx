...
interface TranscriberOptions {
  hmsStore: any;
  setTranscriptAndSpeakingPeer: any;
  setIsTranscriptionEnabled: any;
}

export class Transcriber {
  private hmsStore: any;
  private enabled: boolean;
  private audioSocket: WebSocket | null;
  private setTranscriptAndSpeakingPeer: any;
  private setIsTranscriptionEnabled: any;
  private sttTuningConfig: {
    timeSlice: number;
    desiredSampRate: number;
    numberOfAudioChannels: number;
    bufferSize: number;
  };
  private resetTextTimer: NodeJS.Timeout | null;
  private localPeerName: string;
  private observingLocalPeerTrack: boolean;
  private trackIdBeingObserved: string | null;
  private recordRTCInstance: any;
  ...
  async toggleTranscriptionState(): Promise<void> {
  ...
  setTranscriptAndPeerWithExpiry(transcript: string, peerName: string): void {
    if (!transcript) {
      return;
    }
    this.setTranscriptAndSpeakingPeer(transcript, `[${peerName}]`);
    if (this.resetTextTimer) clearTimeout(this.resetTextTimer);
    this.resetTextTimer = setTimeout(() => {
      this.resetTranscriptAndPeer();
    }, 5000);
  }

  resetTranscriptAndPeer(): void {
    this.setTranscriptAndSpeakingPeer("", "");
    if (this.resetTextTimer) clearTimeout(this.resetTextTimer);
  }

  async listen(retryCount = 0): Promise<void> {
    if (retryCount > 5) {
    ...
    try {
      const url: any =
        process.env.REACT_APP_DYNAMIC_STT_TOKEN_GENERATION_ENDPOINT;
  ...
  async observeLocalPeerTrack(): Promise<void> {
    try {
      ...
      const unsub = this.hmsStore.subscribe(
  ...
  getAndObserveStream = async (): Promise<void> => {
  ...
  async observeStream(stream: MediaStream): Promise<void> {
