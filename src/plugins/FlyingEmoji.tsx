...
const getStartingPoints = (isMobile: boolean) => {
  let arr = [];
  const min = 5;
...
export function FlyingEmoji() {
  ...
  const showFlyingEmoji = useCallback(
    (emojiId: string, senderId: string) => {
  ...
  useEffect(() => {
    (window as any).showFlyingEmoji = showFlyingEmoji;
  }, [showFlyingEmoji]);
      ...
      {emojis.map((emoji) => {
        return (
          <Flex
            ...
            onAnimationEnd={() => {
              setEmojis((emojis) =>
                emojis.filter((item) => item.id !== emoji.id)
              );
            }}
          >
            <Box>
