...
import { HMSHLSPlaybackState } from "@100mslive/hls-player/dist/interfaces/events";

let hlsPlayer: HMSHLSPlayer | null = null;
...
const HLSView = () => {
  ...
  let [hlsStatsState, setHlsStatsState] = useState<HlsPlayerStats>(
    {} as HlsPlayerStats
  );
  const hlsUrl = hlsState.variants[0]?.url;
  const [availableLayers, setAvailableLayers] = useState<HMSHLSLayer[]>([]);
  ...
  const [currentSelectedQuality, setCurrentSelectedQuality] =
    useState<HMSHLSLayer | null>(null);
  ...
  useEffect(() => {
    ...
    const manifestLoadedHandler = ({ layers }: { layers: HMSHLSLayer[] }) => {
      setAvailableLayers(layers);
    };
    const layerUpdatedHandler = ({ layer }: { layer: HMSHLSLayer }) => {
    ...
    const handleError = (data: HMSHLSException) => {
      console.error("[HLSView] error in hls", `${data}`);
    };
    const handleNoLongerLive = ({ isLive }: { isLive: boolean }) => {
      setIsVideoLive(isLive);
    };

    const playbackEventHandler = (data: HMSHLSPlaybackState) =>
      setIsPaused(data.state === HLSPlaybackState.paused);

    const handleAutoplayBlock = (data: HMSHLSException) =>
      setIsHlsAutoplayBlocked(!!data);
    if (videoEl && hlsUrl) {
      ...
      return () => {
        ...
        hlsPlayer?.off(
          HMSHLSPlayerEvents.AUTOPLAY_BLOCKED,
          handleAutoplayBlock
        );
        hlsPlayer?.off(
   ...
   */
  useEffect(() => {
    const onHLSStats = (state: HlsPlayerStats) => setHlsStatsState(state);
    if (enablHlsStats) {
