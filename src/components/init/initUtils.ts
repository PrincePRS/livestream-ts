import { HMSRole } from "@100mslive/react-sdk";
import { identify } from "zipyai";

const zipyKey = process.env.REACT_APP_ZIPY_KEY;

interface ZipyPropsData {
  localPeer: Record<string, any>;
  roomId?: string;
  sessionId: string;
}

export const setUpZipy = ({ localPeer, roomId, sessionId }: ZipyPropsData) => {
  if (!zipyKey) {
    return;
  }
 ...
 */
function canPublishAV(role: HMSRole) {
...
export const normalizeAppPolicyConfig = (
  roleNames: string[],
  rolesMap: Record<string, HMSRole>,
  appPolicyConfig = {}
) => {
  const newConfig: Record<string, any> = Object.assign({}, appPolicyConfig);
