...
const PreviewScreen = React.memo(
  ({ getUserToken }: { getUserToken: (name: string) => Promise<any> }) => {
    const navigate = useNavigation();
    const tokenEndpoint = useTokenEndpoint();
    const [isHeadless, setIsHeadless] = useSetUiSettings(
      UI_SETTINGS.isHeadless
    );
    ...
    const [error, setError] = useState<{ title: string; body: JSX.Element }>({
      title: "",
      body: <></>,
    });
