 ...
 */
const Settings: React.FC<{ setHide: (value: boolean) => void }> = ({
  setHide,
}) => {
  ...
  return (
    <Box className={settingOverflow()}>
      ...
      {audioOutputFiltered?.length && shouldShowAudioOutput && audioOutput ? (
        <DeviceSelector
          title="Speaker"
          icon={<SpeakerIcon />}
          devices={audioOutput}
...
}> = ({ title, devices, selection, onChange, icon, children = null }) => {
...
const TestAudio: React.FC<{ id?: string }> = ({ id }) => {
  const audioRef = useRef<
    HTMLAudioElement & { setSinkId(deviceId: string): void }
  >(null);
