...
const Progress: React.FC<{ type: string; duration?: number }> = ({
  type,
  duration,
}) => {
...
export const PlaylistActive: React.FC<{
  type: string;
  onToggle: (e: any) => void;
}> = ({ type, onToggle }) => {
  ...
  const metaData = active.metadata as any;
  return (
    <Box css={{ mt: "$8" }}>
      <Flex justify="between" css={{ w: "100%" }}>
        <Box>
          ...
          {metaData?.description && (
            <Text variant="xs">{metaData?.description}</Text>
          )}
        </Box>
        <IconButton
          onClick={onToggle}
...
export const AudioPlaylistControls: React.FC<{
  onToggle: (e: any) => void;
}> = ({ onToggle }) => {
...
export const VideoPlaylistControls: React.FC<{
