...
import { HlsPlayerStats } from "@100mslive/hls-player";
...
export function HlsStatsOverlay({
  hlsStatsState,
  onClose,
}: HlsStatsOverlayProps) {
  return (
    <Flex
      css={{
        position: "absolute",
    ...
    >
      ...
      <HlsStatsRow label="Connection speed">
        {`${((hlsStatsState?.bandwidthEstimate ?? 0) / (1000 * 1000)).toFixed(
          2
        )} Mbps`}
      </HlsStatsRow>
      <HlsStatsRow label="Bitrate">
        {`${((hlsStatsState?.bitrate ?? 0) / (1000 * 1000)).toFixed(2)} Mbps`}
      </HlsStatsRow>
      <HlsStatsRow label="distance from live">
        {getDurationFromSeconds((hlsStatsState.distanceFromLive ?? 0) / 1000)}
      </HlsStatsRow>
 ...
 */
export function getDurationFromSeconds(timeInSeconds: number): string {
...
interface HlsStatsRowProps {
  label: string;
  children: ReactNode;
}

const HlsStatsRow = memo(({ label, children }: HlsStatsRowProps) => {
  return (
