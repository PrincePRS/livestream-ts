...
const TileMenu: React.FC<TileMenuProps> = ({
  ...
  const permision = useHMSStore(selectPermissions);

  // const { removeOthers } = useHMSStore(selectPermissions);
  const removeOthers = permision?.removeOthers ?? false;
  const {
    isAudioEnabled,
    isVideoEnabled,
    setVolume,
  ...
  const track = useHMSStore(selectTrackByID(videoTrackID)) as
    | HMSVideoTrack
    | undefined;
            ...
            {audioTrackID ? (
              <StyledMenuTile.VolumeItem data-testid="participant_volume_slider">
                ...
                <Slider
                  ...
                  value={[volume ?? 0]}
                  onValueChange={(e) =>
                    setVolume ? setVolume(e[0]) : console.log
                  }
                />
...
const SimulcastLayers = ({ trackId }: { trackId: string }) => {
  const track = useHMSStore(selectTrackByID(trackId)) as
    | HMSVideoTrack
    | undefined;
  const actions = useHMSActions();
