...
const getCardData = (roleName: string, roomId: string) => {
  let data: any = {};
...
const Card: React.FC<{
  ...
  order?: number;
}> = ({ title, icon, link, content, isHLSRunning, order = 0 }) => {
...
export const HLSStreaming: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <Fragment>
      ...
      <Flex align="center" css={{ p: "$4 $10" }}>
        ...
        <Text variant="tiny" color={"$textMedEmp" as any} css={{ mx: "$8" }}>
...
const EndHLS: React.FC<{ setShowLinks: (value: boolean) => void }> = ({
  setShowLinks,
}) => {
  const hmsActions = useHMSActions();
